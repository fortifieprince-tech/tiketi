...
export async function POST(req: NextRequest) {
  try {
    ...
    if (confirmation.status === 'succeeded') {
      ...
      // ================================================================
      // 🆕 12. GESTION DU PORTEFEUILLE (CRÉDIT DE L'ORGANISATEUR)
      // ================================================================
      ...
      const yabetooFeeRate = 0.05; // 5% fixe (Yabetoo)
      const yabetooFee = Math.round(totalPrice * yabetooFeeRate);
      const platformCommissionRate = (organizer?.commission_rate ?? 0) / 100;
      const platformFee = Math.round(totalPrice * platformCommissionRate);
      const organizerEarning = totalPrice - yabetooFee - platformFee;

      // Créditer le portefeuille de l'organisateur
      if (organizerEarning > 0 && eventData?.organizer_id) {
        await supabase.rpc('increment_wallet_balance', {
          p_organizer_id: eventData.organizer_id,
          p_amount: organizerEarning
        });
      }

      // Enregistrer les frais dans la commande (pour traçabilité)
      // On ajoute les colonnes si elles n'existent pas déjà (on les a ajoutées via SQL)
      await supabase
        .from('orders')
        .update({
      ...
      await supabase
        ...
        .eq('id', order.id)
      throw new Error('Paiement refusé par l\'opérateur mobile')
    }
